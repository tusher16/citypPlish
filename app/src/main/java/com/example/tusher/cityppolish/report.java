...
public class report extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        selBtn = (Button) findViewById(R.id.btnSelect);
        locationManager = (LocationManager) getSystemService(LOCATION_SERVICE);
        locationListener = new LocationListener() {
            @Override
            public void onLocationChanged(Location location) {
                lat = location.getLatitude(); // gets latitude value of user
                lon = location.getLongitude(); // gets longitude value of user
        ...
        btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent cameraintent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE); // camera opens up 
                File file = getFile();
                cameraintent.putExtra(MediaStore.EXTRA_OUTPUT, Uri.fromFile(file)); // image saved to temp memory
        ...
        selBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                selectImg(); // selectImg function referred
            }
        });
    ...
    private File getFile(){
        ...
        if(!folder.exists()){
            folder.mkdir(); // directory created if directory doesn't exist
        }
    ...
    private void selectImg(){
        ...
        intent.setAction(Intent.ACTION_GET_CONTENT); // opens gallery to select image
    ...
    private void uploadImg(){
        ...
        })
        {
            ...
            protected Map<String, String> getParams() throws AuthFailureError {
                ...
                params.put("name", descrip.getText().toString().trim()); // parses description/ name provided by user
                params.put("image",imgToString(bitmap1)); //image passed
                params.put("lat", Double.toString(lat)); // latitude 
                params.put("lon", Double.toString(lon));// longitude
                return params;
            }
        };
    ...
    private String imgToString(Bitmap bitmap1){
        ...
        bitmap1=((BitmapDrawable)imageView.getDrawable()).getBitmap();
        bitmap1.compress(Bitmap.CompressFormat.JPEG, 100, byteArrayOutputStream); // image compressed before uploading
